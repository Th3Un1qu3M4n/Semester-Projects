...
createUser.configure(background = '#415fba')
createUser.geometry('500x400')


def main_menu():
    createUser.destroy()
    subprocess.Popen('pythonw login.pyw')
...
def submit():
    ...
    approveUser='no'
    c.execute("INSERT INTO Audience VALUES(:username, :name, :email, :pNumber, :password, :approved)",
                {
...
caption_label=Label(createUser, text="New User Account", width=30, fg='white', font=('Roman 20 bold'))
...
pNumber_label = Label(createUser, text="Phone Number \n 10 Digit \n (e.g 312123456)", fg='white', font=("Cambria 12 bold"))
pNumber_label['bg']=pNumber_label.master['bg']
pNumber_label.grid(row=5, column=0)
password_label = Label(createUser, text="password", width=10, fg='white', font=("Cambria 12 bold"))
