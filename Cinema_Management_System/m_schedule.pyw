...
for record in records:
    ...
    edit_btn.append(Button(schedule, text="Edit", bg = 'cyan', command=lambda record=record: edit_movie(record[0])))
    edit_btn[count-1].grid(row=imageCount+2, column=8, rowspan=2, ipadx=10)
    delete_btn.append(Button(schedule, text="Delete", bg = 'cyan',  command=lambda record=record: delete_movie(record[0])))
    delete_btn[count-1].grid(row=imageCount+3, column=8, rowspan=2, ipadx=10)
    imageCount+=10
    count += 1
